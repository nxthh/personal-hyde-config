...
_F = {description = "[Window Management|alt-tab window switcher] cycle next (current workspace)"}
hl.bind("ALT + TAB", hl.dsp.window.cycle_next(), _F)
_F = {description = "[Window Management|alt-tab window switcher] cycle previous (current workspace)"}
hl.bind("ALT + SHIFT + TAB", hl.dsp.window.cycle_next({prev = true}), _F)
...
_F = {description = "[Workspaces|Navigation] toggle previous workspace"}
hl.bind(MOD .. " + TAB", hl.dsp.focus({workspace = "previous"}), _F)
...
_F = {description = "[Hardware Controls|Media] toggle play/pause media", locked = true}
hl.bind(MOD .. " + Insert", hl.dsp.exec_cmd("playerctl play-pause"), _F)
...
local TOUCHPAD_NAME = "synps/2-synaptics-touchpad"
local touchpad_enabled = true -- assume touchpad starts enabled
...
local toggle_touchpad = function()
    ...
    hl.dispatch(
        hl.dsp.exec_cmd(
            string.format('notify-send "Touchpad" "%s"', touchpad_enabled and "Enabled" or "Disabled")
        )
    )
end

_F = {description = "[Hardware Controls|Touchpad] toggle touchpad", locked = true}
hl.bind("XF86TouchpadToggle", toggle_touchpad, _F)
hl.bind(MOD .. " + K", toggle_touchpad, _F)

-- Disable middle-click paste globally, keep TrackPoint middle-button-scroll intact
local noop = function() end
_F = {description = "[Hardware Controls|TrackPoint] block middle-click paste", locked = true}
hl.bind("mouse:274", noop, _F)
